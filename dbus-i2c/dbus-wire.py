...
#update W1 temp
def update_W1():
    if not os.path.exists('/sys/bus/w1/devices/28-01205f496e19/hwmon/hwmon2/temp1_input'):
        ...
        fd  = open('/sys/bus/w1/devices/28-01205f496e19/hwmon/hwmon2/temp1_input','r')
        value = float(fd.read())
        value = round(value / 1000.0, 1)
...
#update W1 temp2
def update_W2():
    if not os.path.exists('/sys/bus/w1/devices/28-01145398a8ff/hwmon/hwmon3/temp1_input'):
        if dbusservice['W2-temp']['/Connected'] != 0:
            logging.info("W2 temperature interface disconnected")
            dbusservice['W2-temp']['/Connected'] = 0
    else:
        if dbusservice['W2-temp']['/Connected'] != 1:
            logging.info("W2 temperature interface connected")
            dbusservice['W2-temp']['/Connected'] = 1
        fd  = open('/sys/bus/w1/devices/28-01145398a8ff/hwmon/hwmon3/temp1_input','r')
        value = float(fd.read())
        value = round(value / 1000.0, 1)
        dbusservice['W2-temp']['/Temperature'] = value
        fd.close
...
dbusservice['W2-temp']     = new_service(base, 'temperature', 'Wire',	   '1Wire',  1, 27, 4)

# Tidy up custom or missing items
dbusservice['cpu-temp']   ['/ProductName']     = 'Raspberry Pi'
dbusservice['W1-temp']   ['/ProductName']     = '1Wire Sensor'
dbusservice['W2-temp']   ['/ProductName']     = '1Wire Sensor'
